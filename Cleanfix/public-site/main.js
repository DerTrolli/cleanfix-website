...
(function () {
  ...
  const labelEl     = document.getElementById('dark-toggle-label');
  ...
  function updateIcon() {
    ...
    if (labelEl) labelEl.textContent = isDark ? 'Hellmodus' : 'Dunkelmodus';
...
// then to legacy individual keys, then to Monatsangebot.txt for MA.
(function () {
  ...
  function getBestActive(schedule, type) {
    var active = schedule.filter(function (e) { return e.type === type && isActive(e); });
  ...
  function loadFromLocalStorage() {
    ...
    if (applySchedule(schedule)) return;
  ...
  fetch(DATA_BASE + '/schedule.json')
    ...
    .then(function (schedule) {
      if (!applySchedule(schedule)) {
        // JSON was empty or had no active entries — try localStorage
        loadFromLocalStorage();
      }
    })
    .catch(function () {
      // Fetch failed (offline, file missing) — fall back to localStorage
      loadFromLocalStorage();
    });

})();

// Price tables — fetch JSON first, localStorage fallback, else keep HTML defaults
(function () {
  var tabs = [
    { file: 'preise-reinigung', key: 'cleanfix-preise-reinigung', tbodyId: 'tbody-reinigung', cols: 1 },
    { file: 'preise-buegeln',   key: 'cleanfix-preise-buegeln',   tbodyId: 'tbody-buegeln',   cols: 1 },
    { file: 'preise-waesche',   key: 'cleanfix-preise-waesche',   tbodyId: 'tbody-waesche',   cols: 2 },
  ];

  function renderPrices(d, tab) {
    var tbody = document.getElementById(tab.tbodyId);
    if (!tbody || !d || !d.groups) return;
    var html = '';
  ...
  function loadFromLocalStorage(tab) {
    try {
      var raw = localStorage.getItem(tab.key);
      if (!raw) return;
      renderPrices(JSON.parse(raw), tab);
    } catch (e) {}
  }

  tabs.forEach(function (tab) {
    fetch(DATA_BASE + '/' + tab.file + '.json')
      .then(function (res) {
        if (!res.ok) throw new Error(res.status);
        return res.json();
      })
